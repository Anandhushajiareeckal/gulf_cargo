                    ...
                    <div class="col-sm-12">
                        <div class="card-box table-responsive">
                            ...
                            <table id="datatable"
                                ...
                                @foreach($ship_type as $i=> $ship_type)
                                <tr> 
                                    <td>{{ $i+1}}</td>
                                    <td>{{ $ship_type->name }}</td>
                                    <td>{{ $ship_type->value}} </td>
                                    <td>{{ $ship_type->driver_rate}} </td>
                                    <td>{{ $ship_type->office_rate}} </td>

                                    <form method="post" action="{{delete_url($ship_type->id)}}">
                                            <td>
                                            <a href=" {{edit_url($ship_type->id)}}"
