        ...
        <div class="content">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card-box">
                            <form action="{{ update_url($purchase->id) }}" method="post" enctype="multipart/form-data">
                                @csrf 
                                @method('PUT')
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            ...
                                            <input type="date" max=""
                                                    class="form-control" id="purchase_date" value="{{date('Y-m-d', strtotime($purchase->purchase_date))}}" name="purchase_date"
                                    ...
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="purchase_number" value="{{$purchase->purchase_number}}" class="form-control"
                                                id="purchase_number" readonly>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="branch_id" required>
                                                ...
                                                @foreach($branches as $branch)
                                                    <option value="{{$branch->id}}" @if($branch->id == $purchase->branch_id) selected @endif>{{$branch->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="package col-md-12"  id="package_ID" >
                                            ...
                                            <div class="body" id="packageinfo">
                                                <table class="table table-bordered packageinfo">
                                                    ...
                                                    @foreach($purchaseProducts as $i => $purchaseProduct)
                                                    <?php  
                                                        $j = $i+1;
                                                    ?>
                                                    <tr count="{{$i+1}}">
                                                        ...
                                                        <td width="40%">
                                                            <select name="product_name[]" id="product_name{{$i+1}}" class="form-control product_name"  data-myattri="{{$i+1}}">
                                                                ...
                                                                @foreach($products as $product)
                                                                    <option value="{{$product->id}}" @if($product->id == $purchaseProduct->product_id) selected @endif data-price="{{$product->unit_rate}}">{{$product->product_name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td> 
                                                            <input type="text" placeholder="Enter quantity" name="qty[]" id="product_qty{{$i+1}}" value="{{$purchaseProduct->quantity}}" price="{{$purchaseProduct->amount}}" class="form-control product_qty" data-myattri="{{$i+1}}"> 
                                                        </td>
                                                        <td>
                                                            <input type="text" placeholder="Enter unit price" name="unit[]" id="unit_price{{$i+1}}" value="{{$purchaseProduct->amount}}" readonly class="form-control unit_price" data-myattri="{{$i+1}}">
                                                        </td>
                                                        <td>
                                                            <input type="text" name="subtotal[]" id="subtotal{{$i+1}}" value="{{$purchaseProduct->amount * $purchaseProduct->quantity}}" readonly class="form-control value sub_total" data-myattri="{{$i+1}}">
                                                        </td>
                                                        <td>
                                            ...
                                            <div class="body">
                                                <table class="table table-bordered">
                                                    <tr> 
                                                        ...
                                                        <td width="20%"><h6><span id="totalqty">{{$purchase->total_quantity }}</span></h6></td>
                                                        <td width="20%" class="total"><h6>Total Amount : </h6></td>
                                                        <td width="20%"><h6><span id="totalAmt" class="package-total-amount totalAmt">{{$purchase->total_amount }}</span></h6></td>
                                                        <input type="hidden" name="totalAmount" id="totalAmount"  value="{{$purchase->total_amount }}"/>                                                       
                                                        <input type="hidden" name="totalQuantity" id="totalQuantity" value="{{$purchase->total_quantity }}" />                                                             
                                                    </tr>
                                                </table>
